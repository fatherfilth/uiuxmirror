...
must_haves:
  truths:
    ...
    - "Loading state detected opportunistically via aria-busy=true or classes containing loading/spinner"
    - "Error state detected opportunistically via aria-invalid=true or classes containing error/invalid or red-ish border color"
  ...
  artifacts:
    ...
    - path: "src/components/state-mapper.ts"
      ...
      exports: ["mapComponentStates", "StateMapping", "StateDiff"]
  key_links:
    ...
    - from: "src/components/state-mapper.ts"
      to: "playwright"
      via: "page.locator().hover() and .focus() for state triggering; DOM attribute inspection for loading/error"
      pattern: "hover\\(\\)|focus\\(\\)|aria-busy|aria-invalid"
---

<objective>
Implement component variant detection and interactive state mapping. Variant analysis identifies size, color, and emphasis dimensions. State mapper uses Playwright interactions to capture hover, focus, active, and disabled state styles, plus opportunistic detection of loading and error states from DOM attributes and CSS classes.

Purpose: COMP-02 requires variant dimension detection. COMP-03 requires interactive state mapping per component (hover, active, focus, disabled, loading, error).
...
<task type="auto">
  ...
  <action>
    1. Create `src/components/state-mapper.ts`:
      - `interface StateMapping`: { default: Record<string, string>, hover?: Record<string, string>, focus?: Record<string, string>, active?: Record<string, string>, disabled?: Record<string, string>, loading?: Record<string, string>, error?: Record<string, string> }
      ...
      - `mapComponentStates(page: Page, selector: string): Promise<StateMapping>`
        ...
        d. Disabled state (for button, input, select only):
           ...
           - Compare to default
        e. Loading state (opportunistic -- capture if present, do not trigger):
           - Use page.evaluate() to check: `element.getAttribute('aria-busy') === 'true'` OR element/ancestor has class containing 'loading' or 'spinner'
           - If detected: read tracked properties, compare to default, include diff
           - If not detected: skip (no loading key in StateMapping)
        f. Error state (opportunistic -- capture if present, do not trigger):
           - Use page.evaluate() to check: `element.getAttribute('aria-invalid') === 'true'` OR element has class containing 'error' or 'invalid' OR borderColor is red-ish (parse as RGB, check R > 180 && G < 80 && B < 80)
           - If detected: read tracked properties, compare to default, include diff
           - If not detected: skip (no error key in StateMapping)
        g. For each state, return only properties that changed (diff)
        h. Move mouse away after hover to reset state: `await page.mouse.move(0, 0)`
        i. Blur after focus: `await page.evaluate(() => (document.activeElement as HTMLElement)?.blur())`
        j. Wrap entire function in try/catch -- if interaction fails, return default-only mapping
    ...
    2. Create `tests/components/state-mapper.test.ts`:
      - Unit tests for state diff detection:
        ...
        - Element with aria-busy="true" => loading state detected with style diff
        - Element without aria-busy and no loading class => no loading key in mapping
        - Element with aria-invalid="true" => error state detected with style diff
        - Element with class="input-error" => error state detected
        - Element with red-ish borderColor (rgb(220,38,38)) => error state detected
        - Element with no error indicators => no error key in mapping
  ...
  <done>State mapper triggers hover/focus/disabled via Playwright interaction, detects loading/error states opportunistically from DOM attributes and CSS classes, captures only changed properties, limits to 20 components per type</done>
</task>
...
- Error state detected opportunistically via aria-invalid="true", error/invalid classes, or red-ish border color
