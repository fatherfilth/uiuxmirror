...
<objective>
Wire together the crawler, extractors, evidence store, and storage into a complete pipeline with diff tracking for re-crawls.

Purpose: This is the integration plan that makes everything work end-to-end. A user should be able to run the pipeline with a seed URL and get a .uidna/ directory containing all extracted tokens with full evidence. Running it again should produce a diff showing what changed. This covers the remaining requirements: CRAWL-06 (re-crawl diff) and completing the CRAWL-01 integration.

Output: A runPipeline function that orchestrates crawl -> extract -> store -> (optional) diff, producing a complete .uidna/ output directory.
</objective>
...
<task type="auto">
  ...
  <action>
Create the main pipeline orchestrator that wires crawler -> extractors -> evidence -> storage.
...
c. Define crawler handlers:
   - pageTokenHashes: Record<string, string> = {} (for snapshot)
   - pagesProcessed = 0

   onPageCrawled handler:
   1. Extract tokens: const tokens = await extractAllTokens(page, pageData.url)
      NOTE: The extractors need the Playwright Page object, not just PageData. The crawler's request handler has access to `page`. The orchestrator must modify the crawler integration so extractors run INSIDE the request handler while the page is still open.

      IMPORTANT ARCHITECTURAL DECISION:
      The runCrawl function from Plan 02 uses onPageCrawled(pageData) callback AFTER page processing. But extractors need the live Playwright Page. Two approaches:
      - Option A: Modify runCrawl to also pass the page object to the handler
      - Option B: Run extractors inside a custom request handler

      Use Option A: Modify the CrawlerHandlers interface in src/crawler/playwright-crawler.ts to include the page:
      ```typescript
      onPageCrawled(pageData: PageData, page: Page): Promise<void>
      ```
      Then in the request handler, pass `page` to the callback before navigating away.
...
IMPORTANT: When modifying src/crawler/playwright-crawler.ts to pass the Page object to onPageCrawled, ensure the page is still live (not closed). The extraction must happen BEFORE the page is closed or recycled by Crawlee. Place the extractAllTokens call and evidence storage within the request handler, then call the original onPageCrawled.
  </action>
  <verify>
Run `npx tsc --noEmit` -- should compile with zero errors.
...
Verify output files exist: `ls .uidna/tokens/ .uidna/evidence/ .uidna/snapshots/`
  ...
  <done>Pipeline orchestrates crawl->extract->store->diff in a single runPipeline call. Tokens are persisted per-page and aggregated. Evidence is stored with screenshots. Snapshots enable re-crawl diff detection. End-to-end test against example.com produces valid output.</done>
</task>
...
5. No memory leaks or hanging processes after pipeline completes
</verification>
