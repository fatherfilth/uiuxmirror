...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/crawler/playwright-crawler.ts"
      ...
      exports: ["createCrawler", "runCrawl"]
  key_links:
...
Output: A working crawler that accepts a CrawlConfig, crawls seed URLs respecting all constraints, and returns PageData objects with fully-rendered content ready for token extraction.
</objective>
...
<task type="auto">
  <name>Task 3: Build the main PlaywrightCrawler with Crawlee integration</name>
...
b. CrawlerHandlers interface:
   - onPageCrawled(pageData: PageData): Promise<void> -- called after each page
   - onPageFailed(url: string, error: Error): void -- called on failure
   - onPageSkipped(url: string, reason: string): void -- called when robots blocks
...
- The onPageCrawled handler is the hook where extractors will plug in (Plan 04/05).
...
Run a smoke test: `npx tsx -e "import { runCrawl } from './src/crawler/index.js'; import { loadConfig } from './src/shared/index.js'; const config = loadConfig({ seedUrls: ['https://example.com'], maxPages: 1, maxDepth: 0 }); const result = await runCrawl(config, { onPageCrawled: async (p) => console.log('Crawled:', p.url, 'Status:', p.statusCode), onPageFailed: (u, e) => console.error('Failed:', u, e.message), onPageSkipped: (u, r) => console.log('Skipped:', u, r) }); console.log('Pages processed:', result.pagesProcessed);"` -- should crawl example.com and print status.
  </verify>
  <done>Crawler accepts CrawlConfig, crawls seed URLs with Crawlee PlaywrightCrawler, respects robots.txt, enforces rate limits, uses stealth browser, waits for dynamic content, and delivers PageData through handler callbacks. Single-page crawl of example.com succeeds.</done>
</task>
...
3. RobotsValidator correctly blocks URLs disallowed by robots.txt
4. Framework detection returns 'unknown' for example.com (no framework)
5. Screenshot is saved to .uidna/screenshots/
6. No memory leaks: single page crawl doesn't leave hanging processes
</verification>
